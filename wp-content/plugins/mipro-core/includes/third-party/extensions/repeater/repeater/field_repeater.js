    ...
    redux.field_objects.repeater.init = function( selector ) {
        if ( ! selector ) {
            selector = $( document )
                ...
                .find( '.redux-container-repeater:visible' );
        }
        ...
        $( selector ).each( function( idx ) {
            ...
            var blank = el.find(
                '.redux-repeater-accordion-repeater:last-child'
            );
            redux.repeater[gid].blank = blank
                .clone()
                .wrap( '<p>' )
                .parent()
                .html();
            ...
            if ( 1 == redux.repeater[gid].sortable ) {
                accordian.sortable({
                    axis: 'y',
                    handle: 'h3',
            ...
            el.find( '.redux-repeater-accordion-repeater .bind_title' ).on(
                ...
                function( event ) {
                    ...
                    $( this )
                        .closest( '.redux-repeater-accordion-repeater' )
                        .find( '.redux-repeater-header' )
                        .text( value );
                }
            );
            ...
            el.on( 'click', '.redux-repeaters-remove', function() {
                ...
                var gid = parent.attr( 'data-id' );
                redux.repeater[gid].blank = $( this )
                    .parents( '.redux-repeater-accordion-repeater:first' )
                    .clone( true, true );
                $( this )
                    .parents( '.redux-repeater-accordion-repeater:first' )
                    .slideUp( 'medium', function() {
                        ...
                        if ( '' != redux.repeater[gid].limit ) {
                            var count = parent.find(
                                '.redux-repeater-accordion-repeater'
                            ).length;
                            if ( count < redux.repeater[gid].limit ) {
                                parent
                                    .find( '.redux-repeaters-add' )
                                    .removeClass( 'button-disabled' );
                            }
                        }
                        parent
                            .find(
                                '.redux-repeater-accordion-repeater:last .ui-accordion-header'
                            )
                            .trigger('click');
                    });
            });
            ...
            el.find( '.redux-repeaters-add' ).click( function() {
                ...
                var parent = $( this )
                    .parent()
                    .find( '.redux-repeater-accordion:first' );
                var count = parent.find( '.redux-repeater-accordion-repeater' )
                    .length;
                ...
                var id = parent
                    .find( '.redux-repeater-accordion-repeater' )
                    .size(); // Index number

                if (
                    parent
                        .find( '.redux-repeater-accordion-repeater:last' )
                ...
                if ( redux.repeater[gid]) {
                    redux.repeater[gid].count = el.find(
                        '.redux-repeater-header'
                    ).length;
                    var html = redux.repeater[gid].html.reduxReplaceAll(
                        '99999',
                        id
                    );
                    $( newSlide )
                ...
                newSlide
                    ...
                    .click();
                newSlide
                    .find( '.redux-repeater-accordion-repeater:last .bind_title' )
                ...
                if (
                    0 < redux.repeater[gid].limit &&
                    count >= redux.repeater[gid].limit
                ) {
                    $( this ).addClass( 'button-disabled' );
                }
            });
