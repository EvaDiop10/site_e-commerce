		...
		class ReduxFramework_repeater {
			...
			public function render() {
				...
				if ( isset ( $this->value['redux_repeater_data'] ) && is_array( $this->value['redux_repeater_data'] ) && ! empty ( $this->value['redux_repeater_data'] ) ) {
					...
					foreach ( $repeaters as $repeater ) {
						...
						if ( ! isset( $this->field['static'] ) && empty( $this->field['static'] ) ) {
							echo '<a href="javascript:void(0);" class="button deletion redux-repeaters-remove">' . esc_html__( 'Delete', 'mipro-core' ) . ' ' . $this->field['item_name'] . '</a>';
						}
						echo '</div>';
						echo '</fieldset>';
						echo '</table>';
				...
				if ( ! isset( $this->field['static'] ) && empty( $this->field['static'] ) ) {
					...
					echo '<a href="javascript:void(0);" class="button redux-repeaters-add button-primary' . $disabled . '" rel-id="' . $this->field['id'] . '-ul" rel-name="' . $this->parent->args['opt_name'] . $this->repeater_values . '[title][]">' . esc_html__( 'Add', 'mipro-core' ) . ' ' . $this->field['item_name'] . '</a><br/>';
				}
			...
			function localize( $field, $value = array() ) {
				...
				if ( isset( $field['fields'] ) && ! empty( $field['fields'] ) ) {
					...
					$var = array(
						'html'     => $var . '<a href="javascript:void(0);" class="button deletion redux-repeaters-remove">'. esc_html__( 'Delete', 'mipro-core' ) . '</a>',
						'count'    => count( $value ),
						'sortable' => true,
						'limit'    => '',
